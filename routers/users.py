...
async def get_users(user: user_dependency, db: db_dependency):
    if user is None:
        raise HTTPException(status_code=401, detail="No user loggedin")
...
@router.put("/edit_phone_number/{phone_number}", status_code=status.HTTP_204_NO_CONTENT)
async def edit_phone_number(
